...
type GenerateSocialMediaPostsArgs = {
...
type GenerateSocialMediaPostsResult = {
  posts: string;
  html?: string;
  raw?: unknown;
};

type SocialMediaPostsContent = {
  html: string;
  text: string;
};

export default function SocialMediaPostIdeasPage() {
  ...
  const [postsResult, setPostsResult] = useState<SocialMediaPostsContent | null>(null);
  const [modalError, setModalError] = useState<string | null>(null);
  const [thinkingDots, setThinkingDots] = useState(0);
  const [lastRequest, setLastRequest] = useState<GenerateSocialMediaPostsArgs | null>(null);
  ...
  const postsActionReference = (
    api as unknown as Record<string, any>
  )['functions/generateSocialMediaPosts'].generateSocialMediaPosts as FunctionReference<
    'action',
    'public',
    GenerateSocialMediaPostsArgs,
    GenerateSocialMediaPostsResult
  >;

  const generatePostsAction = useAction(postsActionReference);
  ...
  const runPostGeneration = async (args: GenerateSocialMediaPostsArgs) => {
    setIsGenerating(true);
    setPostsResult(null);
    setModalError(null);

    try {
      const response = await generatePostsAction(args);
      const html = response.html && response.html.trim().length > 0
        ? response.html
        : `<div>${response.posts}</div>`;
      setPostsResult({
        html,
        text: response.posts,
      });
      ...
      const message =
        error instanceof Error
          ? error.message
          : 'Unable to generate social media posts right now. Please try again.';
  ...
  const handleGeneratePostIdeas = () => {
    if (inputMethod === 'topic-verse' && !sermonTopic.trim() && !bibleVerse.trim()) {
      Alert.alert('Missing Information', 'Please provide a sermon topic or Bible verse to generate social media post ideas.');
      return;
    }
    
    if (inputMethod === 'transcription' && !selectedSermon) {
      Alert.alert('Missing Information', 'Please select a sermon to generate social media post ideas.');
      return;
    }

    let args: GenerateSocialMediaPostsArgs;
  ...
  const handleRegeneratePosts = () => {
    ...
    void runPostGeneration(lastRequest);
  };
  ...
  const handleCopyPosts = async () => {
    ...
    try {
      await Clipboard.setStringAsync(postsResult.text);
      Alert.alert('Copied to Clipboard', 'The social media post ideas have been copied to your clipboard.');
    } catch (error) {
      console.error('Failed to copy posts to clipboard', error);
      Alert.alert('Copy Failed', 'Unable to copy the posts. Please try again.');
    }
  };
  ...
  const renderResultBody = (scrollStyle?: StyleProp<ViewStyle>) => (
    <>
      <View style={styles.resultModalHeader}>
        <Text style={styles.resultModalTitle}>Social Media Post Ideas</Text>
        <Pressable
      ...
      {isGenerating && !postsResult && !modalError ? (
        <View style={styles.thinkingContainer}>
          <LoadingIndicator size="large" color={theme.colors.primary} />
          <Text style={styles.thinkingText}>{`Creating Post Ideas${animatedDots}`}</Text>
        </View>
      ) : null}
      ...
      {postsResult ? (
        ...
        >
          <RichHtml html={postsResult.html} />
        </ScrollView>
      ) : null}
      ...
      <View style={styles.resultButtonRow}>
        <Button
          ...
          onPress={handleCopyPosts}
          variant="secondary"
          disabled={!postsResult || isGenerating}
          style={styles.resultButton}
        />
        <Button
          title={isGenerating ? 'Regenerating...' : 'Regenerate Posts'}
          onPress={handleRegeneratePosts}
          variant="primary"
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Social Media Post Ideas</Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Card style={styles.descriptionCard}>
          <View style={styles.toolHeader}>
            ...
            <View style={styles.toolInfo}>
              <Text style={styles.toolTitle}>AI-Powered Social Media Posts</Text>
              <Text style={styles.toolDescription}>
                Generate engaging social media content for your sermons
              </Text>
            </View>
          </View>
        </Card>
        ...
        {inputMethod === 'transcription' && (
          <Card style={styles.inputCard}>
            ...
            <Text style={styles.inputSubtext}>Choose a sermon to generate social media post ideas from</Text>
        ...
        <Card style={styles.generateCard}>
          <Button
            title={isGenerating ? 'Generating Post Ideas...' : 'Generate Social Media Post Ideas'}
            onPress={handleGeneratePostIdeas}
            variant="primary"
        ...
        <Card style={styles.tipsCard}>
          ...
          <View style={styles.tipsList}>
            ...
            <Text style={styles.tipItem}>• Our AI will generate engaging social media posts</Text>
            <Text style={styles.tipItem}>• Posts optimized for different platforms</Text>
            <Text style={styles.tipItem}>• Use for Instagram, Facebook, Twitter, and more</Text>
          </View>
        </Card>
      </ScrollView>
