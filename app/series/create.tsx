...
export default function CreateSeriesScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Simulate API call
      const newSeries: Omit<SermonSeries, 'id' | 'createdAt' | 'updatedAt' | 'sermonCount'> = {
        title: formData.title.trim(),
        description: formData.description.trim() || undefined,
        theme: formData.themes.length > 0 ? formData.themes.join(', ') : undefined,
      ...
      console.log('Creating new series:', newSeries);
      ...
      <View style={styles.inputContainer}>
        ...
        {formData.themes.length > 0 && (
          <View style={styles.selectedThemes}>
            {formData.themes.map((themeText, index) => (
              <View key={index} style={styles.selectedThemePill}>
                <Text style={styles.selectedThemeText}>{themeText}</Text>
                <Pressable
                  onPress={() => {
                    setFormData(prev => ({
                      ...prev,
        ...
        <View style={styles.themeInputContainer}>
          <TextInput
            ...
            maxLength={30}
            onSubmitEditing={() => {
              const trimmedTheme = currentThemeInput.trim();
              if (trimmedTheme && !formData.themes.includes(trimmedTheme)) {
                setFormData(prev => ({
                  ...prev,
                  themes: [...prev.themes, trimmedTheme]
                }));
        ...
        >
          {COMMON_THEMES.filter(commonTheme => !formData.themes.includes(commonTheme)).map((commonTheme) => (
            <Pressable
              key={commonTheme}
              style={styles.themeChip}
              onPress={() => {
                if (!formData.themes.includes(commonTheme)) {
                  setFormData(prev => ({
                    ...prev,
                    themes: [...prev.themes, commonTheme]
                  }));
                }
              }}
            >
              <Text style={styles.themeChipText}>{commonTheme}</Text>
            </Pressable>
          ))}
        </ScrollView>
      </View>
  ...
  const renderPreview = () => (
    <Card style={[styles.section, styles.previewCard]}>
      ...
      <View style={[styles.previewSeries, { borderLeftColor: formData.color }]}>
        <View style={styles.previewHeader}>
          ...
          <View style={styles.previewBadge}>
            <Text style={styles.previewBadgeText}>
              {formData.isActive ? 'Active' : 'Planned'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.previewMeta}>
          ...
          {formData.themes.length > 0 && (
            <Text style={styles.previewMetaText}>• {formData.themes.join(', ')}</Text>
          )}
        </View>
      </View>
    </Card>
